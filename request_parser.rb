  ...
  class << self
    ...
    def call(conn)
      ...
      port = conn.addr[1]
      {
        ...
        "REQUEST_URI" => create_request_uri(
          full_path: full_path,
          port: port,
          remote_host: remote_host
        )
      }.merge(rackalize(headers))
    end

    private

    # リクエストの１行目、例えば"POST /some-path?query HTTP/1.1" <-これを分解する
    ...
    def create_request_uri(full_path:, port:, remote_host:)
